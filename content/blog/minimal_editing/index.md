...
- **rSFT**: Rejection-filtered SFT — train on the completions with the 3 lowest Levenshtein Distances for each sample from the self-distillation dataset.
...
On the first attempt, SFT is almost suspiciously good as the resultant model seems to have perfectly learned the task. I found this extremely surprising and had the initial hypothesis that the model was just memorizing the reversal for the corruptions rather than learning a general minimal editing behavior. As a result, I re-created both synthetic datasets but instead using a completely different set of corruptions than the evaluation set to test for generalization. The core hypothesis was that the model was simply learning to reverse a particular set of corruptions.
...
SFT collapses entirely out-of-domain. Pass@1 drops to 0.458 as the model has learned to make specific minimal changes regardless of whether they fix anything. rSFT and DPO are both better but the overall improvement is slight compared to the 8-shot baseline. RL is the only method that generalizes cleanly, improving on all three metrics over both baselines. The fact that the RL model has larger improvements on Levenshtein Distance and Added Cognitive Complexity than on Pass@1 is further evidence that it is not just memorizing corruption reversals but actually learning a general minimal editing behavior.
...
Taken together, the results suggest that over-editing is both widespread and measurable. At the same time, the prompting results show that this is not purely a capability limitation. Especially for reasoning models, a simple instruction to preserve the original code leads to much more faithful edits, which is an encouraging sign that when models do over-edit, they can still be steered toward higher-quality code.

More encouragingly, the training results suggest that this behavior can be improved more systematically. Reinforcement learning produced more faithful editors without the same degradation in general coding ability, and those gains held up across both the 4B and 14B Qwen3 models.
...
However, in my experience, Over-Editing is a very real and common problem that exists across all frontier coding models today. It has existed for some time now and has always been pretty hard to quantify and evaluate in the real-world. I hope that this work can serve as a first step towards establishing a framework for evaluating and improving the minimal editing capability of coding models, thereby improving the overall quality of code generated by AI.
