...
date = 2026-05-10
[params]
subtitle = "On forgetting, generalization, and what connects RL to on-policy distillation"
math = true
+++

<figure class="post-figure">
    <img
      src="images/distributions_cover.png"
      alt="RL distribution"
      loading="lazy"
      decoding="async"
    />
...
<figure class="post-figure post-figure--wide">
  <a class="post-figure__link" href="images/sft_distribution.svg">
    <img
      src="images/sft_distribution.svg"
      alt="Starting model distribution versus dataset/SFT target: dense gray dashed arrows show uniform SFT pressure; teal arrows show useful updates toward minimal editing behavior; rust arrows show collateral pressure on syntax, reasoning, and other regions."
      loading="lazy"
      decoding="async"
    />
  </a>
...
<figure class="post-figure post-figure--wide">
  <a class="post-figure__link" href="images/rl_distribution.svg">
    <img
      src="images/rl_distribution.svg"
      alt="RL distribution"
      loading="lazy"
      decoding="async"
    />
...
<figure class="post-figure">
  <a class="post-figure__link" href="images/opsd_token.png">
    <img src="images/opsd_token.png" alt="" loading="lazy" decoding="async" />
  </a>
  <figcaption>In all thinking configurations, style tokens have significantly higher per-token KL divergence while Math tokens have the lowest KL divergence. </figcaption>
</figure>
...
A common argument is that RLVR with outcome rewards struggles with per-token credit-assignment and the advantage function provides only O(1) bits of information per episode ([Schulman et al., 2025](https://thinkingmachines.ai/blog/lora/)). OPSD seems to be on the opposite extreme where each token gets its unique 'reward' but the algorithm introduces more noise and bias per update.

## On-Policy Distillation With Different Teachers 
The setup for this experiment uses my Minimal Code Editing task. I highly encourage reading my [previous post](https://nrehiew.github.io/blog/minimal_editing/) to get some context before continuing here. As a quick summary, the model is provided with a buggy corrupted function and asked to fix the bug and nothing else. We check if the model's solution works and if any other part of the function has been rewritten against the original uncorrupted function. I like this environment because it helps test for 2 things:
  ...
  <div class="post-table__wrap">
    <table>
      ...
      <tbody>
        ...
        <tr><td>SFT</td><td>0.775</td><td>0.450</td><td>0.450</td><td>0.286</td></tr>
        <tr><td>RL</td><td>0.792</td><td>0.063</td><td>0.206</td><td>0.320</td></tr>
      </tbody>
      <tbody>
        <tr class="post-table__section">
          <th colspan="5">Students (OPD)</th>
        </tr>
        <tr><td>OPD SFT teacher</td><td><strong>0.800</strong></td><td>0.059</td><td><strong>0.206</strong></td><td>0.297</td></tr>
        <tr><td>OPD RL teacher</td><td>0.787</td><td><strong>0.055</strong></td><td>0.228</td><td><strong>0.314</strong></td></tr>
      </tbody>
    </table>
  </div>
  <figcaption>Teachers vs. on-policy distillation students on the minimal editing task (out-of-domain corruptions) and LiveCodeBench v6.</figcaption>
</figure>
...
The mode-covering behavior of forward KL could thus lead to sacrificing previous modes (representing pre-existing capabilities) to learn the new task. [Chen et al., 2025](https://arxiv.org/abs/2510.18874) showed that RL can be thought of as reverse KL minimization.

<figure class="post-figure">
  <a class="post-figure__link" href="images/toy_reverseKL.png">
    <img src="images/toy_reverseKL.png" loading="lazy" decoding="async" />
  </a>
  <figcaption>In a toy multi-modal setting, the authors show that reverse KL forgets less than forward KL.</figcaption>
</figure>
...
<section class="context-figure" style="padding: 0.95rem 1.1rem;">
  <header style="font-weight: 600; font-size: 0.92rem; margin: 0 0 0.45rem 0; color: #b4102d;">A Note on Distributional Shaping</header>
  <p style="margin: 0; font-size: 0.95rem; line-height: 1.65; color: #383838;">This perspective of thinking about training as changing the shape of the model distribution can be unintuitive. One example is this work by <a href="https://arxiv.org/abs/2604.01193">Zhang et al., 2026</a>. The authors report coding improvements when trained on a dataset consisting of 1 model-generated completion per prompt with no correctness filtering. Even more surprising is that when they self-distill using high temperature - resulting in gibberish training data, training on this poor quality data still improves performance. The authors attempt to explain the results by comparing probability outputs at certain tokens but the point is that it is extremely challenging for one to build intuition around these high dimensional things.
  </p>
</section>

My best attempt at an explanation here is that the model is experiencing mode collapse around the new capability. Looking at entropy statistics during OPD and RL, the entropy collapse is significantly more drastic with OPD. This is to be expected with the mode-seeking behavior of reverse KL training where mode-collapse might lead to decreased diversity ([Gu et al., 2023](https://arxiv.org/abs/2306.08543)). I do not want to overstate this since this section is speculative but I do think the distributional view is the right place to start.

<figure class="post-figure post-figure--wide">
  <a class="post-figure__link" href="images/reward_entropy_plots.svg">
    <img src="images/reward_entropy_plots.svg" loading="lazy" decoding="async" />
  </a>
  <figcaption>Reward and Entropy Training curves of OPD vs RL. RL increases reward gradually while with OPD, the reward increase is a lot more sudden and corresponds with a drastic collapse in entropy. </figcaption>
</figure>
...
Crucially, we can borrow the work of [Ross et al., 2010](https://arxiv.org/abs/1011.0686). In SFT, the model only sees states the teacher visits. At test time because of autoregression, one mistake by the student might move it to states the teacher would have never visited. This mismatch can result in compounding errors when the student's prefix (or current state) differs from what the teacher would have done. As such, on-policy data aggregation can reduce this mismatch. 

## The Full Pipeline
Most (open) models use a Pretrain-SFT-RL-OPD pipeline. SFT is required after pretraining for both format adherence and to instill basic instruction following capability, without which it would be impossible to perform RL efficiently ([Chu et al., 2025](https://arxiv.org/abs/2501.17161)). But what happens after? 

Recently, both [GLM 5](https://arxiv.org/abs/2602.15763) and [DeepSeek v4](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/blob/main/DeepSeek_V4.pdf) use OPD to merge expert capabilities into the final model with the final checkpoint never going through RL. This begs the following question: how should we train these experts? A useful reference point to compare across methods is the [MiMo-V2 Flash technical report](https://arxiv.org/abs/2601.02780).

<figure class="post-figure">
  <a class="post-figure__link" href="images/mimo_v2_flash.png">
    <img src="images/mimo_v2_flash.png" loading="lazy" decoding="async" />
  </a>
  <figcaption>Table 7 from the MiMo-V2-Flash Technical Report lists the post-training approach that leads to the best teacher model across domains.</figcaption>
</figure>
